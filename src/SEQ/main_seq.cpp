...
// void calcDensity_loop_cells(double x_min, double x_max, double y_min, double y_max, 
//                     int N, double* x, double* y, int M, int* A) {
//     int tmp_row, tmp_col;
//     double tmp_x, tmp_y;
//     // save width of cell
//     double cell_width = (x_max - x_min)/(double)M;
//     double cell_height = (y_max - y_min)/(double)M;
//     // initialise the number of points found
//     int points_found = 0;
//     // find the cell in which the point falls
//     for (int i = 0; i < M; i++) {
//         for (int j = 0; j < M; j++) {
//             // only check cells if there are still points to find
//             if (points_found < N) {
//                 for (int point_id = 0; point_id < N; point_id++) {
//                     // save point coordinate
//                     tmp_x = x[point_id];
//                     tmp_y = y[point_id];
//                     // check if point falls in cell's x- and y-range
//                     if (((x_min+(double)i*cell_width  <= tmp_x) && (tmp_x < x_min+(double)(i+1)*cell_width)) && 
//                         ((y_min+(double)j*cell_height <= tmp_y) && (tmp_y < y_min+(double)(j+1)*cell_height))) { 
//                             A[i*M + j]++;
//                             points_found++;
//                     }
//                 }
...
void calcDensity_loop_points(double x_min, double x_max, double y_min, double y_max, 
                    int N, double* x, double* y, int M, int* A) {
    int tmp_row, tmp_col;
    double tmp_x, tmp_y;
    // save width of cell
...
int main() {
    ...
    int* mat_points_loop = (int*)malloc(bytes_mat);
    int* mat_cells_loop = (int*)malloc(bytes_mat);
    std::cout << "\n";
    ...
    std::string param_set_string_cells = "cells_a="+stream_a.str() + "_b="+stream_b.str() + "_c="+stream_c.str() + "_d="+stream_d.str();
    std::string param_set_string_points = "points_a="+stream_a.str() + "_b="+stream_b.str() + "_c="+stream_c.str() + "_d="+stream_d.str();
    ...
    for (int i = 0; i < SIZE_MATRIX; i++) {
        for (int j = 0; j < SIZE_MATRIX; j++) {
            mat_cells_loop[i*SIZE_MATRIX + j] = (int)0;
            mat_points_loop[i*SIZE_MATRIX + j] = (int)0;
        }
    }
    ...
    // // loop over cells
    // std::cout << "Calculating density map (looping over cells)...\n";
    // auto start_cells = std::chrono::high_resolution_clock::now();
    // calcDensity_loop_cells(x_range.min, x_range.max, y_range.min, y_range.max, 
    //                     NUM_POINTS, points_x, points_y, SIZE_MATRIX, mat_cells_loop);
    // auto end_cells = std::chrono::high_resolution_clock::now();
    // std::chrono::duration<double> elapsed_cells = end_cells - start_cells;
    // std::cout << "Sequential computation elapsed: " << elapsed_cells.count() << " seconds\n";
    // std::cout << "\n";

    // calculate the density map on the CPU sequentially
    // loop over points
    std::cout << "Calculating density map (looping over points)...\n";
    auto start_points = std::chrono::high_resolution_clock::now();
    calcDensity_loop_points(x_range.min, x_range.max, y_range.min, y_range.max, 
                        NUM_POINTS, points_x, points_y, SIZE_MATRIX, mat_points_loop);
    auto end_points = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed_points = end_points - start_points;
    std::cout << "Sequential computation elapsed: " << elapsed_points.count() << " seconds\n";
    std::cout << "\n";
    ...
    // std::ofstream mydensity_cells("data_density_" + param_set_string_cells + ".txt");
    // if (mydensity_cells.is_open()) {
    //     for (int i = 0; i < SIZE_MATRIX; i++) {
    //         for (int j = 0; j < SIZE_MATRIX; j++) {
    //             if (j < SIZE_MATRIX-1) {
    //                 mydensity_cells << mat_cells_loop[i*SIZE_MATRIX + j] << ", ";
    //             } else {
    //                 mydensity_cells << mat_cells_loop[i*SIZE_MATRIX + j] << "\n";
    //             }
    //         }
    //     }
    //     mydensity_cells.close();
    // } else {
    //     std::cout << "ERROR: Unable to open density file.\n";
    //     throw std::runtime_error("ERROR: Unable to open density file.\n");
    // }

    // std::ofstream mydensity_points("data_density_" + param_set_string_points + ".txt");
    // if (mydensity_points.is_open()) {
    //     for (int i = 0; i < SIZE_MATRIX; i++) {
    //         for (int j = 0; j < SIZE_MATRIX; j++) {
    //             if (j < SIZE_MATRIX-1) {
    //                 mydensity_points << mat_points_loop[i*SIZE_MATRIX + j] << ", ";
    //             } else {
    //                 mydensity_points << mat_points_loop[i*SIZE_MATRIX + j] << "\n";
    //             }
    //         }
    //     }
    //     mydensity_points.close();
    // } else {
    //     std::cout << "ERROR: Unable to open density file.\n";
    //     throw std::runtime_error("ERROR: Unable to open density file.\n");
    // }
